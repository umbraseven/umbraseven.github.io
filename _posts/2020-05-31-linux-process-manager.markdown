...
《Understanding Linux Kernel》中给进程的定义是：
>一个进程可以定义为：“程序执行时的一个实例”，或者一个运行程序的“执行上下文”。
>从内核的观点看，进程的目的就是担当分配系统资源的实体。

文件系统的定义：
>从这个意义上说，文件系统是硬盘分区物理组织的用户级视图。
...
链表是最简单的数据结构，事实上它是顺序遍历最适合的结构。因为遍历时间不可能小于O(n)。而链表组织简单，维护容易，插入方便。在linux内核中主要使用的链表是环形双向链表，这样在首尾插入或者查询时灵活性更高。
...
从内核组织的角度来看，进程会存在于哪些位置？对于running进程，它会存在于runqueue中，在active/expired两个array之一。

当进程进入睡眠时呢？存在于等待队列。还有其他地方会持有它的task指针吗？linux0.11中，有一个task数组；2.6好像没有了。
...
eip的切换更有意思，其中的一个细节是，调用__wtich_to()时用的jmp而非call，为什么？因为虽然在大多数时候，我们都是返回标号1，但是某些时候不是。
...
对于第一项，有一系列的、相互冲突的需求，比如响应时间、吞吐量、公平……对于第二项，最重要的是足够快，其实是足够简单，多占点内存反而没关系。前者一般是策略相关的，后者则是算法相关的。当然两者是互相影响的——决定一个策略是否可行的，除了策略本身之外，还依赖于是否有良好的算法实现。
...
（关于睡眠、等待、唤醒、唤醒函数、进程数据结构等，需要整理为一个小专题：等待队列）
